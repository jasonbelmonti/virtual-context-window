  ...
  const executeTurn = async (
    ...
    const onStreamEvent = executeOptions?.streamEvents
      ? async (event: AssistantGenerateStreamEvent) => {
          ...
          const sanitizedPreview = await sanitizeStreamingPreview(
            streamedRawContent,
          );
          if (sanitizedPreview.length <= emittedVisibleChars) {
            return;
          }
          const safeDelta = sanitizedPreview.slice(emittedVisibleChars);
          emittedVisibleChars = sanitizedPreview.length;
    ...
    if (
      ...
      const remainingDelta = sanitized.content.slice(emittedVisibleChars);
      await executeOptions.streamEvents({
        type: "assistant_text_delta",
        threadId,
